...
随着大语言模型逐步从单轮问答扩展到长时多轮交互，基于模型构建能够持续感知环境、保持用户画像并完成复杂任务的智能体系统，已经成为人工智能应用的重要发展方向。在这一过程中，长期记忆能力逐渐从“可选增强模块”转变为“系统核心基础设施”。尤其在跨会话对话、长期个性化服务、复杂任务规划和多阶段协同场景下，若系统无法稳定保存并调用历史信息，就很容易出现事实遗忘、上下文断裂、人格漂移、重复提问和答非所问等问题。

从工程实现角度看，当前长期记忆系统已经形成了多种技术路线，例如基于检索增强生成的方案、基于摘要压缩的方案、基于时间层级结构组织的方案，以及引入反思与重要性评分的混合记忆方案。这些方案在系统结构与性能表现上差异明显，但它们都会面临一个共同问题：当智能体回答错误时，开发者往往只能看到“结果错了”，却很难进一步判断问题究竟发生在记忆写入、原生检索还是答案生成环节。若评测系统缺乏细粒度归因能力，后续优化就只能依赖经验试错，难以形成高效、可复用的系统调试闭环。
...
\subsection{长期记忆与长上下文建模研究}

在长文本和多轮交互任务中，最直观的解决思路是扩大上下文窗口，但这种方法通常伴随着较高的推理成本与注意力稀释问题。Lewis 等提出的检索增强生成方法 [2] 为知识密集型任务提供了新的外部记忆使用范式，成为后续大量记忆系统的重要基础。随着智能体研究的推进，ReAct [3] 将推理与行动统一起来，Toolformer [4] 强调模型的工具调用学习能力，Generative Agents [5]、Reflexion [6] 和 MemGPT [7] 等工作则进一步把长期记忆、经验反思与任务规划纳入统一框架，使记忆从简单的上下文缓存演变为智能体持续运行的重要组成部分。

\subsection{长期记忆基准与评测研究}

长期记忆系统的发展同样依赖系统化评测。LoCoMo 数据集 [1] 面向超长多会话对话记忆，覆盖问答、事件摘要等任务，是当前长期对话记忆评测的重要代表。除此之外，LongMemEval [10]、MemoryBench [11]、StoryBench [12]、A-MEMGYM [13] 与 HaluMem [14] 等工作也分别从长期交互、多轮持续学习、动态叙事一致性、交互式记忆基准以及记忆系统幻觉等角度推进了记忆评测研究。TiMem [9] 则展示了基于时间层级整合的记忆框架在 LoCoMo 和 LongMemEval-S 上的有效性，为“时间结构优先”的记忆组织理念提供了新的实证支持。

\subsection{现有工作不足}

综合已有研究可以发现，当前长期记忆评测仍存在三个突出问题。第一，很多方案仍以最终正确率作为核心指标，无法细分错误来源。第二，部分评测方法过度依赖特定系统内部结构，难以统一适配异构记忆系统，导致横向比较困难。第三，在真实工程场景中，环境冲突、接口差异、运行超时和中间结果不可见等问题会显著影响实验复现，而传统论文对这些问题关注不足。

因此，本文将长期记忆评测定位为一个“方法设计 + 工程系统实现”的综合问题，而不仅是单一的判分问题。
...
\begin{enumerate}
    ...
    \item 设计统一适配器协议，并接入 O-Mem 与 MemBox 两类真实系统；
...
本文所评测的对象是“具备长期记忆能力的智能体系统”。在这一场景中，系统需要处理跨会话、多时间跨度的历史对话，并在后续查询到来时检索并组织相关信息，最终生成与上下文一致的答案。与只评估模型最终回答文本的传统问答任务不同，本文关心的是整个长期记忆链路是否可靠，即：原始信息是否被正确写入、是否在回答时被正确检索、是否被生成模块忠实利用。
...
之所以必须区分这两类任务，是因为它们对系统的要求并不相同。POS 任务强调“记得住且找得到”，NEG 任务强调“没有依据时不应凭空作答”。若使用统一规则处理两类任务，会使系统在“该答”和“不该答”两个维度上的行为被混淆，削弱评测结论的解释力。
...
通过这组统一编码，系统错误不再被压缩成单一“答错”，而是能够映射到更明确的责任位置。

\section{统一语义判分机制}

在长期对话场景中，仅用字符串完全匹配判断答案是否正确存在明显局限。例如，同义表达、时间格式改写、简称与全称转换都可能导致正确答案被误判为错误。为此，本文在实现中引入统一 CorrectnessJudge，将规则判分与大模型语义判分结合起来，用于 baseline 与 eval 的统一口径。TiMem 对语义判定提示词的组织方式 [9] 为本文设计语义判分提示提供了重要参考。
...
其中，$final\_correct(q_i)$ 表示第 $i$ 个问题在统一语义判分后的最终正确性。除总体正确率外，本文还统计缺陷分布。设缺陷集合为 $D$，则缺陷 $d$ 的出现频次可表示为：
...
在规则层面，本文遵循“先编码、后检索、再生成”的责任传导原则。例如，当编码层已经为 \texttt{MISS} 时，检索层的 \texttt{RF} 不再被视为主要故障；只有编码层非缺失而检索层未命中时，\texttt{RF} 才成立。该机制可以有效避免错误归因层层叠加。
...
编码探针用于回答“原始对话中的关键信息是否被系统正确写入记忆库”。对 POS 任务，它需要判断目标事实是否存在、是否模糊以及是否写错；对 NEG 任务，它需要检查系统是否误写入了本不该存在的伪记忆。
...
检索探针关注的问题是“系统在在线回答时，是否真正取回了应当使用的记忆证据”。与编码探针不同，检索探针关注的是运行时原生检索输出，而非记忆库是否存在目标记录。

对于 POS 任务，若关键证据被成功检出，则状态为 \texttt{HIT}；若未命中，则状态为 \texttt{MISS}；若虽然命中但存在大量误导性噪声，则可视为 \texttt{NOISE}。对于 NEG 任务，检索探针重点观察系统是否返回足以诱导回答的无关信息，此时可归为 \texttt{NIR}。该设计使“检索失败”与“编码失败”可以被明确区分。

\section{生成探针设计}

生成探针关注的是“系统在给定上下文条件下是否生成了正确且忠实的答案”。为了分离检索误差和生成误差，本文同时比较在线回答与 oracle 回答。在线回答来自真实系统运行路径，oracle 回答则在提供理想证据上下文条件下生成。
...
综合三探针与归因代理，本文评测主流程如算法 \ref{alg:main-eval} 所示。

\begin{algorithm}[htbp]
\caption{长期记忆系统统一评测流程}
\label{alg:main-eval}
\begin{algorithmic}[1]
\REQUIRE 标准样本集合 $Q$，适配器 $A$，评测配置 $C$
\ENSURE 逐题结果、运行摘要与最终归因报告
\FOR{每个样本 $q \in Q$}
    \STATE 根据样本所属会话准备或复用系统运行上下文
    \STATE 并行执行编码探针、检索探针和生成探针
    \STATE 使用统一语义判分模块计算最终正确性
    \STATE 调用最终归因代理融合三个探针的状态与缺陷
    \STATE 将逐题结果写入结构化 JSON 文件
\ENDFOR
\STATE 汇总运行摘要、错误索引和缺陷统计
\end{algorithmic}
\end{algorithm}
...
O-Mem 适配器的核心目标是最大限度保留原系统原生行为，包括原生记忆写入、原生检索和原生生成路径。在实际工程中，O-Mem 面临的一个突出问题是环境依赖链复杂，例如 \texttt{flash-attn}、\texttt{torch} 与 \texttt{transformers} 版本不匹配时，可能在嵌入模型加载阶段直接报错。若没有统一适配层与结构化脚本，这类问题极易被误判为“评测框架不可用”。

针对上述问题，本文采用 stable-eval 变体作为优先评测对象，并通过适配器层显式暴露模型路径、API 凭据与运行配置参数，从而把环境问题与评测逻辑问题解耦，提高了 O-Mem 的复现稳定性。

\section{MemBox 适配实现}

MemBox 的一个典型特点是建库过程较重，且内部会生成较多中间文件。若每次评测都重新 build，不仅重复开销较大，而且很容易把 build 过程误判为 eval 卡死。为此，本文将 MemBox 链路显式拆分为 \texttt{build}、\texttt{baseline} 与 \texttt{eval} 三个阶段，并允许通过 manifest 复用 build artifact [18]。
...
这一设计不仅降低了重复建库成本，还增强了实验的阶段可观测性，使开发者更容易区分问题究竟发生在 build 还是 eval 环节。
...
\begin{enumerate}
    \item 统一评测框架能否在 O-Mem 与 MemBox 上稳定运行；
    \item 多探针归因是否能够比单一正确率提供更细粒度的失效解释；
    \item baseline 与 eval 在统一语义判分下能否保持一致的统计口径；
    \item build/eval 分离与逐题落盘是否能够提升实验可观测性与复现效率。
\end{enumerate}

\section{实验环境与数据配置}

实验数据集采用 LoCoMo 子集 \texttt{data/locomo10.json}。针对不同系统，实验推荐分别使用独立运行环境，以避免依赖冲突。O-Mem 需要额外指定嵌入模型路径；MemBox 推荐使用专属环境并采用 build artifact 复用模式。
...
项目 & 配置说明 \\
\midrule
数据集 & LoCoMo 子集 \texttt{data/locomo10.json} \\
O-Mem 环境 & 独立 conda 环境，需配置嵌入模型路径 \\
MemBox 环境 & 独立 conda 环境，推荐采用 build/eval 分离模式 \\
API 凭据 & 使用本地配置文件统一管理 \\
输出格式 & 聚合 JSON + run summary + question index + per-question JSON \\
\bottomrule
\end{tabular}
\end{table}
...
实验至少应包含以下比较维度：O-Mem 与 MemBox 的 baseline 正确率比较、O-Mem 与 MemBox 的 eval 缺陷分布比较、baseline 与 eval 在统一 CorrectnessJudge 下的一致性比较、MemBox 在 build/eval 分离前后的重复运行效率比较，以及编码层引入外部高召回检索器前后的观测覆盖变化比较。

由于当前定量实验结果尚未全部补齐，本文严格不编造数值，而是保留标准化结果记录模板，待后续实验完成后直接补入。
...
\begin{enumerate}
    \item 最终正确率，即基于统一语义判分的 \texttt{final\_correct}；
    \item 探针状态分布，即编码、检索、生成三层状态占比；
    \item 缺陷频次，即各缺陷编码的出现次数；
    \item 运行错误率，即 \texttt{EVAL\_ERROR} 的发生比例；
    \item 复现效率，即在工件复用条件下的重复评测成本变化。
\end{enumerate}

\section{结果记录模板}

为保证后续实验结果补充时结构统一，本文预先给出结果表模板。

\begin{table}[htbp]
\centering
\caption{系统总体结果对比表（待补）}
\label{tab:overall-result}
\begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.18\textwidth}p{0.18\textwidth}}
\toprule
系统 & Baseline 最终正确率 & Eval 最终正确率 & EVAL\_ERROR 比例 \\
\midrule
O-Mem StableEval & 待补充 & 待补充 & 待补充 \\
MemBox StableEval & 待补充 & 待补充 & 待补充 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\caption{探针缺陷分布表（待补）}
\label{tab:defect-result}
\begin{tabular}{p{0.2\textwidth}p{0.15\textwidth}p{0.15\textwidth}p{0.15\textwidth}p{0.15\textwidth}}
\toprule
系统 & 编码层缺陷 & 检索层缺陷 & 生成层缺陷 & 主归因分布 \\
\midrule
O-Mem StableEval & 待补充 & 待补充 & 待补充 & 待补充 \\
MemBox StableEval & 待补充 & 待补充 & 待补充 & 待补充 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{MemBox build/eval 分离收益表（待补）}
\label{tab:membox-benefit}
\begin{tabular}{p{0.22\textwidth}p{0.2\textwidth}p{0.2\textwidth}p{0.2\textwidth}}
\toprule
方案 & 首次运行耗时 & 重复评测耗时 & 可观察性 \\
\midrule
build 与 eval 混合 & 待补充 & 待补充 & 待补充 \\
build/eval 分离 & 待补充 & 待补充 & 待补充 \\
\bottomrule
\end{tabular}
\end{table}
...
当实验结果补齐后，本文建议从以下路径展开分析：首先比较总体正确率，判断不同系统在统一语义判分下的整体差异；其次比较探针状态与缺陷分布，定位各系统主要瓶颈；再次针对异常高的 \texttt{RF}、\texttt{GRF} 或 \texttt{EVAL\_ERROR} 深入分析逐题 JSON；最后结合系统实现细节，提出针对性的优化建议。

\section{本章小结}

本章给出了完整的实验目标、配置、评价指标与结果记录模板。由于当前量化结果仍在持续补充，本文保留了标准化空白位置而不虚构数值，以保证论文内容真实、严谨且后续可直接更新。

\chapter{案例分析与工程讨论}

\section{O-Mem 适配案例分析}

在 O-Mem 的真实适配过程中，一个典型问题是深度学习依赖链的不稳定。例如，嵌入模型加载阶段可能受到 \texttt{flash-attn}、\texttt{torch} 与 \texttt{transformers} 版本兼容关系影响，从而在系统启动阶段直接报错。若没有统一适配层与结构化日志，这类问题往往会被误判为“评测框架不可用”。

本文实现的统一评测框架并不掩盖这些真实运行问题，而是通过适配器层、运行脚本和错误记录机制，把环境问题、运行问题与评测问题分开记录。这样，开发者能够区分“系统未能正常启动”和“系统启动后回答错误”两类问题，从而更高效地组织调试流程。

\section{MemBox build/eval 分离案例分析}

MemBox 的一个核心工程问题是建库过程较重、耗时较长。若直接把 build 和 eval 混合在同一个长任务中，用户往往只能看到“任务长时间没有输出”，难以判断当前处于正常建库、异常卡住还是在线评测超时状态。本文通过 build artifact 的引入，把这一问题显式拆分。

在该设计下，用户可以先确认 build 是否成功生成 manifest，再复用该工件运行 baseline 和 eval。若 eval 阶段仍然耗时异常，则可以进一步通过逐题心跳日志和逐题结果文件判断问题发生在具体哪一道题。由此，系统问题被从“整体不可见”转化为“阶段可区分、逐题可定位”的可观察问题。
...
尽管本文框架已经能够覆盖 O-Mem 与 MemBox 等真实系统，但仍存在若干局限：
...
本章结合 O-Mem 与 MemBox 的典型工程案例，讨论了统一评测框架在真实开发和问题定位中的价值。可以看出，本文方法不仅服务于最终实验统计，更能在系统开发与调试阶段持续发挥作用。

\chapter{总结与展望}

\section{本文总结}

本文围绕长期记忆智能体评测问题，设计并实现了一套面向真实系统的统一评测框架。与只关注最终正确率的传统方法不同，本文从编码、检索、生成三个阶段对系统行为进行并行诊断，并通过最终归因代理形成简洁、稳定的责任判断。在工程实现层面，本文构建了统一数据契约与适配器协议，完成了 O-Mem 与 MemBox 的真实接入，实现了 baseline、build、eval 分离链路、统一 CorrectnessJudge 语义判分、逐题 JSON 落盘与结构化日志输出等机制。

从论文贡献角度看，本文工作主要体现在三个方面：第一，提出了一套面向长期记忆智能体的三探针并行归因评测框架；第二，实现了可扩展的统一适配器层，使异构记忆系统能够接入同一评测协议；第三，给出了完整的工程实现、实验设计与结果留存方案，为后续补充量化实验奠定基础。

尽管当前部分定量实验仍在持续补充，本文已经完成了论文所需的方法设计、工程实现和实验框架搭建，并为后续结果更新预留了完整位置。

\section{进一步展望}

围绕本文工作，未来可以从以下方向继续推进：

\begin{enumerate}
    \item 将更多记忆系统接入统一适配器层，例如图记忆、向量数据库记忆和混合记忆系统；
    \item 将评测数据扩展到真实用户日志、任务型对话和多模态场景；
    \item 将归因规则与学习型诊断方法结合，提高复杂错误链分析能力；
    \item 引入更系统的统计分析、置信区间与重复实验机制，提高实验报告的严谨性；
    \item 基于逐题诊断结果，形成自动化调参或自动化修复建议，实现从评测到优化的闭环。
\end{enumerate}

综上，本文完成的不是某一单一记忆模型的再实现，而是一套可持续扩展的长期记忆评测基础设施。随着后续实验补齐与适配系统增多，该框架有望进一步发展为面向长期记忆智能体研究的通用实验平台。
