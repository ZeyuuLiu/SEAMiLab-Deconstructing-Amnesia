...
from utils import OpenAIClient, gpt_generate_answer, gpt_extract_theme, gpt_update_profile, gpt_generate_multi_summary, get_timestamp, llm_extract_keywords, gpt_personality_analysis, get_default_chat_model
import re
import openai
import time
import tiktoken
...
def generate_system_response_with_meta(query, short_mem, long_mem, retrieval_queue, long_konwledge, client, sample_id, speaker_a, speaker_b, meta_data):
    ...
    response = client.chat_completion(model=get_default_chat_model(), messages=messages, temperature=0.7, max_tokens=2000)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run MemoryOS LoCoMo parsing and answer generation")
    parser.add_argument("--dataset", default="locomo10.json", help="Path to locomo dataset json")
    parser.add_argument("--output", default="all_loco_results.json", help="Output result json path")
    parser.add_argument("--memory-dir", default="mem_tmp_loco_final", help="Directory for temporary memory files")
    parser.add_argument("--sample-id", default="", help="Optional sample_id filter")
    parser.add_argument("--limit", type=int, default=0, help="Optional number of samples to process")
...
def main():
    ...
    try:
        ...
        print(f"错误：找不到数据文件 {args.dataset}")
        return
    ...
    if args.sample_id:
        dataset = [sample for sample in dataset if sample.get("sample_id") == args.sample_id]
    ...
    for idx, sample in enumerate(dataset):
        ...
        short_mem = ShortTermMemory(max_capacity=1, file_path=f"{args.memory_dir}/{sample_id}_short_term.json")
        mid_mem = MidTermMemory(max_capacity=2000, file_path=f"{args.memory_dir}/{sample_id}_mid_term.json")
        long_mem = LongTermMemory(file_path=f"{args.memory_dir}/{sample_id}_long_term.json")
