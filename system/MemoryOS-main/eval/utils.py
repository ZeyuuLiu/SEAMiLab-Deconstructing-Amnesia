...
def get_runtime_settings():
    return {
        "api_key": os.getenv("MEMORYOS_API_KEY", os.getenv("OPENAI_API_KEY", "")),
        "base_url": os.getenv("MEMORYOS_BASE_URL", os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1")),
        "chat_model": os.getenv("MEMORYOS_CHAT_MODEL", os.getenv("OPENAI_MODEL", "gpt-4o-mini")),
        "embedding_model": os.getenv("MEMORYOS_EMBED_MODEL", "all-MiniLM-L6-v2"),
    }
...
def get_embedding(text, model_name=None):
    if model_name is None:
        model_name = get_runtime_settings()["embedding_model"]
...
class OpenAIClient:
    ...
    def from_env(cls):
        settings = get_runtime_settings()
        return cls(api_key=settings["api_key"], base_url=settings["base_url"])


def get_default_chat_model():
    return get_runtime_settings()["chat_model"]


def gpt_generate_answer(prompt, messages, client):
    return client.chat_completion(model=get_default_chat_model(), messages=messages, temperature=0.7, max_tokens=2000)
