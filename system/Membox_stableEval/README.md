# Membox_stableEval: Stable-Eval Fork for Attribution Experiments

This folder is a **local stable-eval fork** of the original `system/Membox` code.

Goal:

1. preserve the original Membox memory method and prompts as much as possible
2. avoid touching the original `Membox` folder
3. apply only **runtime / output stability fixes** needed for baseline reproduction and our attribution framework

Current local fix set:

1. `Config.apply_run_id()` now also rewrites `TRACE_STATS_FILE`
   - this prevents `FileNotFoundError: [Errno 2] No such file or directory: ''`
   - no retrieval / generation / prompting logic is changed

If you want the original paper-path behavior, use `system/Membox`.
If you want the version intended for local baseline + eval runs in this repo, use `system/Membox_stableEval`.
