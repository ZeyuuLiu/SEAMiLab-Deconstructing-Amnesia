...
class Episodic_Memory:
    ...
    async def evolve_topic_episodic_memory(self):
        ...
        for cluster in topic_clusters:
            ...
            current_results = await call_llm_json_with_retries(
                self.client,
                self.model,
                episodic_memory_merge_message,
                temperature=0.3,
                retries=5,
                retry_delay_sec=1.0,
                label="evolve_topic_episodic_memory",
                validator=lambda obj: require_keys(obj, ["Grouped Topics"], "grouped topics payload"),
            )
