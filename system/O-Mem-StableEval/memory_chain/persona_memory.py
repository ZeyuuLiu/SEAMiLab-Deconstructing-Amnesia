...
class Persona_Memory:
    ...
    async def update_preference_persona(self, episodic_topic_memory_list):
        ...
        for cluster in topic_clusters:
            ...
            persona_preference = await call_llm_json_with_retries(
                self.client,
                self.llm_model,
                preference_generate_message,
                temperature=0.3,
                retries=5,
                retry_delay_sec=1.0,
                label="update_preference_persona",
            )
    ...
    async def update_attribute_persona(self, episodic_memory_attr_dict):
        ...
        for cluster in attr_clusters:
            ...
            persona_attr = await call_llm_json_with_retries(
                self.client,
                self.llm_model,
                fact_generate_message,
                temperature=0.1,
                retries=5,
                retry_delay_sec=1.0,
                label="update_attribute_persona",
            )
