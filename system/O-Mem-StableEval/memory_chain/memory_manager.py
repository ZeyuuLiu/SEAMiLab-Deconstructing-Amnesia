...
class MemoryManager:
    ...
    async def receive_message(self,message,index,client,timestamp,user_speak):
        understanding = call_llm_json_with_retries(
            self.llm_client,
            self.llm_model,
            [{"role": "user", "content": UNDERSTAND_USER_EXPERIENCE_PROMPT.format(message=message)}],
            retries=5,
            retry_delay_sec=1.0,
            label="receive_message understanding",
            validator=lambda obj: require_keys(obj, ["summary", "tags"], "understanding payload"),
        )
        understanding = await understanding
        tags = understanding.get("tags", {}) if isinstance(understanding, dict) else {}
        require_keys(tags, ["topic", "attitude", "reason", "facts", "attributes"], "understanding tags")
        topics_list = list(tags.get("topic", []))
        emotions_list = list(tags.get("attitude", []))
        reasons_list = list(tags.get("reason", []))
        facts_list = list(tags.get("facts", []))
        attributes = tags.get("attributes", [])
        if not topics_list or not emotions_list or not reasons_list or not facts_list:
            raise StableEvalJSONError("understanding payload has empty required tag lists")
        understanding_with_index = "["+str(index)+"]: "+ str(understanding.get("summary", "")).strip()
    ...
    async def understand_dialogue(self,message,index,client):
        understanding = await call_llm_json_with_retries(
            client,
            self.llm_model,
            [{"role": "user", "content": UNDERSTAND_USER_EXPERIENCE_PROMPT_v4.format(message=message)}],
            retries=5,
            retry_delay_sec=1.0,
            label="understand_dialogue",
            validator=lambda obj: require_keys(obj, ["summary", "tags"], "dialogue understanding payload"),
        )
        tags = understanding.get("tags", {}) if isinstance(understanding, dict) else {}
        require_keys(tags, ["topic", "attitude", "reason"], "dialogue understanding tags")
        if not tags.get("topic") or not tags.get("attitude") or not tags.get("reason"):
            raise StableEvalJSONError("dialogue understanding payload has empty required tag lists")
    ...
    async def wm_to_em_router(self,message_with_understanding,episodic_event_memory_profile,client):
        ...
        router_decision = await call_llm_json_with_retries(
            client,
            self.llm_model,
            self.create_messages_for_update_episodic_event_memory_concerning_new_message(
                episodic_event_memory=episodic_event_memory_profile,
                message=message_with_understanding,
            ),
            retries=5,
            retry_delay_sec=1.0,
            label="wm_to_em_router",
            validator=lambda obj: require_keys(obj, ["Action", "Target"], "event router payload"),
        )
    ...
    async def wm_to_em_router_fact(self,message_with_understanding,episodic_fact_memory_profile,client):
        ...
        router_decision = await call_llm_json_with_retries(
            client,
            self.llm_model,
            self.create_messages_for_update_episodic_fact_memory_concerning_new_message(
                episodic_fact_memory=episodic_fact_memory_profile,
                message=message_with_understanding,
            ),
            retries=5,
            retry_delay_sec=1.0,
            label="wm_to_em_router_fact",
            validator=lambda obj: require_keys(obj, ["Action", "Target"], "fact router payload"),
        )
    ...
    async def wm_to_em_router_attr(self,message_with_understanding,episodic_attr_memory_profile,client):
        ...
        router_decision = await call_llm_json_with_retries(
            client,
            self.llm_model,
            self.create_messages_for_update_episodic_attr_memory_concerning_new_message(
                episodic_attr_memory=episodic_attr_memory_profile,
                message=message_with_understanding,
            ),
            retries=5,
            retry_delay_sec=1.0,
            label="wm_to_em_router_attr",
            validator=lambda obj: require_keys(obj, ["Action", "Target"], "attr router payload"),
        )
        action = router_decision["Action"]
        target = router_decision["Target"]
        print("Action: "+action+" happen concerning "+target)
    ...
    async def generate_system_response(self,query,restrieval_result,client,speaker_a,speaker_b,llm_model):
        ...
        response = await call_llm_text_with_retries(
            client,
            llm_model,
            messages,
            temperature=0.7,
            ...
            retries=5,
            retry_delay_sec=1.0,
            label="generate_system_response",
        )
